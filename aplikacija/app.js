...
async function spletnaSeja(sila){
  ...
  var r;
  try{ r = sila ? await c.auth.refreshSession() : await c.auth.getSession(); }catch(e){ throw new Error("ni povezave"); }
...
async function spletVstop(){
  ...
  try{
    ...
    else { prikaziSpletnoPrijavo(jeOmrezna(e) ? "Ni povezave z internetom." : "Seja je potekla — prijavite se znova."); return; }
  }
...
/* brezPina: samodejni vstop zadnjega uporabnika ob zagonu tablice (koda le ob menjavi) */
async function izberiProfil(em, tile, brezPina){
  ...
  var naprej = brezPina ? vstopi : vstopiSPinom;
  try{
...
(async function init(){
  try{
    ...
    if(zadnji && await izberiProfil(zadnji.email, null, true)) return;
