...
(function () {
  ...
  function meni() {
    const deli = OSEBJE ? [['domov', 'Pregled'], ['arhiv', 'Arhiv'], ['fakture', 'Fakture'], ['stranke', 'Stranke'], ['uvoz', 'Uvoz'], ['aplikacija', 'Aplikacija'], ['uporabniki', 'Uporabniki'], ['racun', 'Moj račun']] : [['domov', 'Pregled'], ['arhiv', 'Arhiv'], ['katalog', 'Katalog'], ['racun', 'Moj račun']];
  ...
  async function shraniNovaStranka(box) {
    ...
    msg.textContent = 'Shranjujem …';
    try {
      const { data: novOrg, error: e1 } = await sb.from('orgs').insert({ name: naziv, legal_name: podjetje || null, address: naslov || null, vat_id: davcna || null }).select('id,name,legal_name,address,vat_id').single();
      if (e1) throw e1;
      if (artikli.length) {
