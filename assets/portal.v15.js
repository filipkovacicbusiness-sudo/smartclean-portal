...
(function () {
  ...
  function meni() {
    const deli = OSEBJE ? [['domov', 'Pregled'], ['arhiv', 'Arhiv'], ['fakture', 'Fakture'], ['stranke', 'Stranke'], ['uvoz', 'Uvoz'], ['aplikacija', 'Aplikacija'], ['uporabniki', 'Uporabniki'], ['prijave', 'Prijave'], ['racun', 'Moj račun']] : [['domov', 'Pregled'], ['arhiv', 'Arhiv'], ['katalog', 'Katalog'], ['racun', 'Moj račun']];
  ...
  async function nalozifakture() {
    ...
    let q = sb.from('delivery_notes')
      .select('id,number,doc_date,weight_kg,total_pieces,org_id,delivery_note_items(article_name,pieces)')
    ...
    notes.forEach(n => {
      const oid = n.org_id || '—';
      if (!poOrg[oid]) poOrg[oid] = { org_id: oid, listov: 0, kg: 0, kosov: 0, artikli: {} };
      const g = poOrg[oid];
      g.listov++;
    ...
    list.querySelectorAll('[data-fakprint]').forEach(b => b.addEventListener('click', () => natisniFakturo(parseInt(b.dataset.fakprint, 10))));
  }
  ...
  function fakKartica(g, gi) {
    ...
    const rows = arts.length ? arts.map(([nm, q]) => `<tr><td>${escape_(nm)}</td><td class="q">${stevilo(q)}</td></tr>`).join('') : '<tr><td colspan="2" class="u-sub">Brez postavk</td></tr>';
    return `<div class="fak-card">
      <div class="fak-card-h">
        <div><h3>${escape_(ime)}</h3><p class="u-sub">${stevilo(g.listov)} spremnih listov · ${stevilo(g.kosov)} kosov · ${fakKg(g.kg)}</p></div>
        <button type="button" class="btn-mini" data-fakprint="${gi}">Natisni</button>
      </div>
      <table class="fak-t"><thead><tr><th>Artikel</th><th class="q">Količina</th></tr></thead>
        <tbody>${rows}</tbody>
  ...
  function natisniFakturo(gi) {
    ...
    const html = `<!DOCTYPE html><html lang="sl"><head><meta charset="utf-8"><title>Osnova za račun · ${escape_(naziv)}</title><style>
      ...
      *{box-sizing:border-box;font-family:-apple-system,'Segoe UI',Roboto,Arial,sans-serif;color:#16202b}
      ...
      <div class="client"><b>Naročnik storitve:</b> ${escape_(naziv)}${naslov ? '<br>' + escape_(naslov) : ''}<div class="dates">Obdobje: ${datum(FAK_ZADNJI.od)} – ${datum(FAK_ZADNJI.doo)} · ${stevilo(g.listov)} spremnih listov</div></div>
  ...
  /* ══════════ DNEVNIK PRIJAV ══════════ */
  async function risiPrijave() {
    const box = $('prijaveList');
    if (!box) return;
    box.innerHTML = '<div class="rows"><div class="empty">Nalagam …</div></div>';
    const { data, error } = await sb.from('device_logins').select('user_name,login_at,logout_at,duration_sec').order('login_at', { ascending: false }).limit(300);
    if (error) {
      box.innerHTML = '<div class="panel"><p class="u-sub">Napaka: ' + escape_(error.message) + '<br>Je tabela <b>device_logins</b> že ustvarjena v Supabase? (zaženi dnevnik_prijav.sql)</p></div>';
      return;
    }
    const rows = data || [];
    if ($('prijavePod')) $('prijavePod').textContent = rows.length ? (rows.length + ' zadnjih prijav') : 'Ni zabeleženih prijav';
    if (!rows.length) {
      box.innerHTML = '<div class="rows"><div class="empty"><h3>Ni prijav</h3><p>Ko se osebje prijavi na tablico, se bo prijava izpisala tukaj.</p></div></div>';
      return;
    }
    const trajanje = s => { s = s || 0; const m = Math.floor(s / 60); return m + 'm ' + (s % 60) + 's'; };
    box.innerHTML = '<div class="rows" style="padding:8px 16px"><table class="fak-t"><thead><tr><th>Uporabnik</th><th>Prijava</th><th class="q">Trajanje</th></tr></thead><tbody>' +
      rows.map(l => {
        const d = l.login_at ? datumcas(l.login_at) : '—';
        return `<tr><td>${escape_(l.user_name || '—')}</td><td>${escape_(d)}</td><td class="q">${trajanje(l.duration_sec)}</td></tr>`;
  ...
  function render() {
    ...
    $('content').innerHTML = '<div class="rows">' + list.map((o, i) => {
      ...
      <span class="bar" title="${fmtKg(kg)} ta mesec"><i style="width:${Math.round(kg / naj * 100)}%"></i></span>
