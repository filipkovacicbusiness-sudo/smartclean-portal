...
(function () {
  ...
  async function naloziRoleCfg() {
    ...
    // To je lahko širše ali ožje od nastavljenega, zato razlog vedno zabeležimo.
    try {
      ...
      // Zapisa ni — to je normalno pred prvo nastavitvijo vlog.
    } catch (e) {
      ROLE_CFG = null;
  ...
  async function naloziListe() {
    ...
    VSEHLISTOV = count || 0;
    // Liste s starim zapisom (postavka brez povezave na artikel → article_id IS NULL)
    // beremo V OZADJU: to je pregled CELE tabele postavk in je daleč najdražja
    // poizvedba ob prijavi. Rabimo jo le za oznako »star zapis« in števec
    // nepotrjenih, torej sme priti sekundo kasneje — prijava je s tem hitrejša.
    STAR_SET = new Set();
  ...
  function naloziStarSet() {
    ...
    _starP = (async function () {
      try {
        ...
        STAR_SET = set;
        // Podatek je prišel za prvim izrisom → osveži pogleda, ki ga prikazujeta.
  ...
  /* pdfgen.js se naloži ŠELE ob prvi uporabi (prihrani ~80 kB ob vsakem zagonu portala).
     CSP dovoli script-src 'self', zato je dinamično vstavljena skripta z istega strežnika v redu. */
  var _pdfP = null;
  ...
  // Vrne true, ko je PDF modul pripravljen; sicer uporabniku pove, zakaj ne gre.
  ...
  // Logotip je datoteka (smartclean-logo.png), ne vgrajen base64 — portal.js je s tem ~66 kB manjši
  // ob vsakem zagonu. Absolutni URL, ker tiskalno okno (about:blank) relativnih poti ne razreši.
